...
<div class="estados-container">
    <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 24px;">
        ...
        <h2 style="margin: 0; color: #1e293b;">Nueva SesiÃ³n</h2>
    </div>
    ...
    <div style="background: white; border-radius: 16px; padding: 30px; box-shadow: 0 4px 20px rgba(0,0,0,0.06);">
        <form method="POST" action="{{ route('sesiones.store') }}">
            ...
            {{-- ACOMPAÃ‘ANTE (solo para adolescente/niÃ±o) --}}
            <div id="acompananteSection" style="display: none; margin-bottom: 16px;">
                <label style="display: block; font-weight: 600; color: #475569; margin-bottom: 6px;">Â¿QuiÃ©n lo acompaÃ±Ã³?</label>
                <select name="acompanante_id" id="acompananteSelect" style="width:100%;padding:10px 14px;border:2px solid #e2e8f0;border-radius:10px;font-size:14px;">
                    <option value="">Sin acompaÃ±ante</option>
                </select>
                <small style="color: #64748b; font-size: 11px;">Solo aplica para pacientes adolescentes y niÃ±os</small>
            </div>
            ...
            <div style="margin-bottom: 16px;">
                ...
                <textarea name="resumen" rows="4" required placeholder="Resumen general de la sesiÃ³n (mÃ­nimo 5 caracteres)..." style="width:100%;padding:10px 14px;border:2px solid {{ $errors->has('resumen') ? '#ef4444' : '#e2e8f0' }};border-radius:10px;font-size:14px;">{{ old('resumen') }}</textarea>
            ...
            <div style="margin-bottom: 16px;">
                ...
                <textarea name="observaciones_clinicas" rows="3" placeholder="Observaciones clÃ­nicas relevantes..." style="width:100%;padding:10px 14px;border:2px solid #e2e8f0;border-radius:10px;font-size:14px;">{{ old('observaciones_clinicas') }}</textarea>
            </div>
            <div style="display: flex; gap: 10px; justify-content: flex-end;">
                ...
                <button type="submit" style="padding:10px 24px;background:linear-gradient(135deg,var(--sipce-primary),var(--sipce-primary-dark));color:white;border:none;border-radius:10px;font-weight:600;cursor:pointer;">Crear SesiÃ³n</button>
            </div>
        </form>
    </div>
</div>
...
function cargarAcompanantes() {
    ...
    if (tipo === 'adolescente' || tipo === 'niÃ±o') {
        acomSection.style.display = 'block';
        acomSelect.innerHTML = '<option value="">Sin acompaÃ±ante</option>';
        try {
