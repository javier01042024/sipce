        ...
        <div class="card-body-custom">
            <form action="{{ route('pacientes.update', $paciente) }}" method="POST" id="formEditPaciente" onsubmit="return validarFormEdit()">
                    ...
                    <div class="grid-form">
                        ...
                        <div class="field">
                            ...
                            <select name="tipo_atencion" class="input-field" required>
                                ...
                                <option value="publico" {{ old('tipo_atencion', $paciente->tipo_atencion) == 'publico' ? 'selected' : '' }}>PÃºblico</option>
                            </select>
                        </div>
                        <div class="field">
                            <label>Estado</label>
                        ...
                        <div class="field full-width">
                            <label>DiagnÃ³stico Preliminar</label>
                            <textarea name="diagnostico_preliminar" class="input-field" rows="3" placeholder="DiagnÃ³stico inicial o impresiÃ³n clÃ­nica...">{{ old('diagnostico_preliminar', $paciente->diagnostico_preliminar) }}</textarea>
                        </div>
                    </div>
                </div>

                {{-- ============================================= --}}
                {{-- CAMPOS ESPECÃFICOS SEGÃšN TIPO DE PACIENTE --}}
                {{-- ============================================= --}}

                @php
                    // PROTECCIÃ“N: Si no hay detalle, usar objeto vacÃ­o para evitar errores
                ...
                <div id="adultoFields">
                    ...
                    <div class="grid-form">
                        <div class="field"><label>Nombre *</label><input type="text" name="nombre" class="input-field" value="{{ old('nombre', $detalle->nombre ?? '') }}" required oninput="this.value = this.value.replace(/[^a-zA-ZÃ¡Ã©Ã­Ã³ÃºÃÃ‰ÃÃ“ÃšÃ±Ã‘\s]/g, '')"></div>
                        <div class="field"><label>Apellido *</label><input type="text" name="apellido" class="input-field" value="{{ old('apellido', $detalle->apellido ?? '') }}" required oninput="this.value = this.value.replace(/[^a-zA-ZÃ¡Ã©Ã­Ã³ÃºÃÃ‰ÃÃ“ÃšÃ±Ã‘\s]/g, '')"></div>
                        <div class="field"><label>CÃ©dula *</label>
                        ...
                        <div class="field"><label>GÃ©nero</label><select name="genero" class="input-field"><option value="">Seleccione...</option><option value="Masculino" {{ old('genero', $detalle->genero ?? '') == 'Masculino' ? 'selected' : '' }}>Masculino</option><option value="Femenino" {{ old('genero', $detalle->genero ?? '') == 'Femenino' ? 'selected' : '' }}>Femenino</option><option value="Otro" {{ old('genero', $detalle->genero ?? '') == 'Otro' ? 'selected' : '' }}>Otro</option></select></div>
                        <div class="field"><label>Estado Civil</label><select name="estado_civil" class="input-field"><option value="">Seleccione...</option>@foreach(['Soltero/a', 'Casado/a', 'Divorciado/a', 'Viudo/a', 'UniÃ³n Libre'] as $ec)<option value="{{ $ec }}" {{ old('estado_civil', $detalle->estado_civil ?? '') == $ec ? 'selected' : '' }}>{{ $ec }}</option>@endforeach</select></div>
                        <div class="field"><label>TelÃ©fono *</label><input type="text" name="telefono" class="input-field" value="{{ old('telefono', $detalle->telefono ?? '') }}" required oninput="this.value = this.value.replace(/[^0-9+]/g, '').slice(0, 15)"></div>
                        <div class="field"><label>Correo electrÃ³nico</label><input type="email" name="email" class="input-field" value="{{ old('email', $detalle->email ?? '') }}"></div>
                        <div class="field full-width"><label>DirecciÃ³n</label><input type="text" name="direccion" class="input-field" value="{{ old('direccion', $detalle->direccion ?? '') }}"></div>
                        <div class="field"><label>Nivel de InstrucciÃ³n</label><select name="nivel_instruccion" class="input-field"><option value="">Seleccione...</option>@foreach(['Primaria', 'Secundaria', 'Universitario', 'Postgrado', 'Otro'] as $ni)<option value="{{ $ni }}" {{ old('nivel_instruccion', $detalle->nivel_instruccion ?? '') == $ni ? 'selected' : '' }}>{{ $ni }}</option>@endforeach</select></div>
                    ...
                    <div class="grid-form">
                        <div class="field"><label>OcupaciÃ³n</label><input type="text" name="ocupacion" class="input-field" value="{{ old('ocupacion', $detalle->ocupacion ?? '') }}"></div>
                        ...
                        <div class="field"><label>TelÃ©fono Trabajo</label><input type="text" name="telefono_trabajo" class="input-field" value="{{ old('telefono_trabajo', $detalle->telefono_trabajo ?? '') }}"></div>
                    </div>
                    ...
                    <div class="grid-form">
                        ...
                        <div class="field"><label>TelÃ©fono</label><input type="text" name="telefono_emergencia" class="input-field" value="{{ old('telefono_emergencia', $detalle->telefono_emergencia ?? '') }}"></div>
                    ...
                    <div class="grid-form">
                        <div class="field full-width"><label>Motivo de consulta</label><textarea name="motivo_consulta" class="input-field" rows="3" placeholder="Â¿QuÃ© te trajo a consulta? Â¿QuÃ© te gustarÃ­a que cambiara?">{{ old('motivo_consulta', $detalle->motivo_consulta ?? '') }}</textarea></div>
                        ...
                        <div class="field full-width"><label>Estrategias de Afrontamiento</label><textarea name="estrategias_afrontamiento" class="input-field" rows="2" placeholder="Â¿QuÃ© haces normalmente cuando te sucede?">{{ old('estrategias_afrontamiento', $detalle->estrategias_afrontamiento ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿QuÃ© sientes en tu cuerpo?</label><textarea name="sensaciones_corporales" class="input-field" rows="2" placeholder="Ej: tensiÃ³n, palpitaciones, opresiÃ³n en el pecho...">{{ old('sensaciones_corporales', $detalle->sensaciones_corporales ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿Alguna vez has pensado en hacerte daÃ±o?</label><textarea name="autolesiones" class="input-field" rows="2">{{ old('autolesiones', $detalle->autolesiones ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿Has pensado en la muerte o en quitarte la vida?</label><textarea name="ideacion_suicida" class="input-field" rows="2">{{ old('ideacion_suicida', $detalle->ideacion_suicida ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿Has consumido alcohol o drogas?</label><textarea name="consumo_sustancias" class="input-field" rows="2">{{ old('consumo_sustancias', $detalle->consumo_sustancias ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Estrategias previas usadas</label><textarea name="estrategias_previas" class="input-field" rows="2" placeholder="Â¿QuÃ© has intentado hacer para sentirte mejor?">{{ old('estrategias_previas', $detalle->estrategias_previas ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿QuÃ© te gustarÃ­a lograr con la terapia?</label><textarea name="objetivos_terapia" class="input-field" rows="2">{{ old('objetivos_terapia', $detalle->objetivos_terapia ?? '') }}</textarea></div>
                    </div>
                    ...
                    <div class="grid-form">
                        <div class="field full-width"><label>Â¿Con quiÃ©n vives?</label><textarea name="convivencia" class="input-field" rows="2">{{ old('convivencia', $detalle->convivencia ?? '') }}</textarea></div>
                        <div class="field full-width"><label>RelaciÃ³n Familiar</label><textarea name="relacion_familiar" class="input-field" rows="3">{{ old('relacion_familiar', $detalle->relacion_familiar ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿CÃ³mo manejas los conflictos familiares?</label><textarea name="conflictos_familiares" class="input-field" rows="2">{{ old('conflictos_familiares', $detalle->conflictos_familiares ?? '') }}</textarea></div>
                    ...
                    <div class="grid-form">
                        ...
                        <div class="field full-width"><label>Problemas en el Ã¡rea sexual</label><textarea name="problemas_sexuales" class="input-field" rows="2">{{ old('problemas_sexuales', $detalle->problemas_sexuales ?? '') }}</textarea></div>
                    ...
                    <h4 style="color: #2d3748; margin: 25px 0 15px; font-size: 16px;"><i class="fas fa-brain" style="color: var(--sipce-primary);"></i> EvaluaciÃ³n Emocional y Conductual</h4>
                    <div class="grid-form">
                        <div class="field full-width"><label>Estado de Ã¡nimo</label><textarea name="estado_animo" class="input-field" rows="2">{{ old('estado_animo', $detalle->estado_animo ?? '') }}</textarea></div>
                        <div class="field full-width"><label>SÃ­ntomas depresivos</label><textarea name="sintomas_depresivos" class="input-field" rows="3">{{ old('sintomas_depresivos', $detalle->sintomas_depresivos ?? '') }}</textarea></div>
                        <div class="field full-width"><label>SÃ­ntomas de ansiedad</label><textarea name="sintomas_ansiedad" class="input-field" rows="3">{{ old('sintomas_ansiedad', $detalle->sintomas_ansiedad ?? '') }}</textarea></div>
                        ...
                        <div class="field full-width"><label>SÃ­ntomas fÃ­sicos</label><textarea name="sintomas_fisicos" class="input-field" rows="2">{{ old('sintomas_fisicos', $detalle->sintomas_fisicos ?? '') }}</textarea></div>
                    ...
                    <div class="grid-form">
                        <div class="field full-width"><label>Antecedentes PsiquiÃ¡tricos</label><textarea name="antecedentes_psiquiatricos" class="input-field" rows="3">{{ old('antecedentes_psiquiatricos', $detalle->antecedentes_psiquiatricos ?? '') }}</textarea></div>
                        ...
                        <div class="field full-width"><label>HÃ¡bitos de SueÃ±o</label><textarea name="habitos_sueno" class="input-field" rows="2">{{ old('habitos_sueno', $detalle->habitos_sueno ?? '') }}</textarea></div>
                    </div>
                    ...
                    <div class="grid-form">
                        <div class="field full-width"><label>Â¿QuÃ© haces bien?</label><textarea name="fortalezas" class="input-field" rows="2">{{ old('fortalezas', $detalle->fortalezas ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿QuÃ© te da orgullo?</label><textarea name="logros" class="input-field" rows="2">{{ old('logros', $detalle->logros ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿QuiÃ©n te respalda?</label><textarea name="red_apoyo" class="input-field" rows="2">{{ old('red_apoyo', $detalle->red_apoyo ?? '') }}</textarea></div>
                        <div class="field"><label>Escala 0-10: Â¿CuÃ¡nto quieres que cambien las cosas?</label><input type="number" name="motivacion_cambio" class="input-field" min="0" max="10" value="{{ old('motivacion_cambio', $detalle->motivacion_cambio ?? '') }}"></div>
                        <div class="field full-width"><label>3 valores + 1 acciÃ³n pequeÃ±a</label><textarea name="valores_accion" class="input-field" rows="3">{{ old('valores_accion', $detalle->valores_accion ?? '') }}</textarea></div>
                ...
                <div id="adolescenteFields">
                    ...
                    <div class="grid-form">
                        <div class="field"><label>Nombre *</label><input type="text" name="nombre" class="input-field" value="{{ old('nombre', $detalle->nombre ?? '') }}" required oninput="this.value = this.value.replace(/[^a-zA-ZÃ¡Ã©Ã­Ã³ÃºÃÃ‰ÃÃ“ÃšÃ±Ã‘\s]/g, '')"></div>
                        <div class="field"><label>Apellido *</label><input type="text" name="apellido" class="input-field" value="{{ old('apellido', $detalle->apellido ?? '') }}" required oninput="this.value = this.value.replace(/[^a-zA-ZÃ¡Ã©Ã­Ã³ÃºÃÃ‰ÃÃ“ÃšÃ±Ã‘\s]/g, '')"></div>
                        <div class="field"><label>CÃ©dula</label><input type="text" name="cedula" class="input-field" value="{{ old('cedula', $detalle->cedula ?? '') }}"></div>
                        <div class="field"><label>Fecha de Nacimiento *</label><input type="text" name="fecha_nacimiento" class="input-field" value="{{ old('fecha_nacimiento', isset($detalle->fecha_nacimiento) ? \Carbon\Carbon::parse($detalle->fecha_nacimiento)->format('d/m/Y') : '') }}" placeholder="DD/MM/AAAA" required oninput="formatearFecha(this)" maxlength="10" autocomplete="off"></div>
                        <div class="field"><label>GÃ©nero</label><select name="genero" class="input-field"><option value="">Seleccione...</option>@foreach(['Masculino', 'Femenino', 'Otro'] as $g)<option value="{{ $g }}" {{ old('genero', $detalle->genero ?? '') == $g ? 'selected' : '' }}>{{ $g }}</option>@endforeach</select></div>
                        <div class="field"><label>TelÃ©fono Personal</label><input type="text" name="telefono_personal" class="input-field" value="{{ old('telefono_personal', $detalle->telefono_personal ?? '') }}"></div>
                        ...
                        <div class="field"><label>InstituciÃ³n Educativa</label><input type="text" name="institucion_educativa" class="input-field" value="{{ old('institucion_educativa', $detalle->institucion_educativa ?? '') }}"></div>
                        ...
                        <div class="field full-width"><label>DirecciÃ³n</label><input type="text" name="direccion" class="input-field" value="{{ old('direccion', $detalle->direccion ?? '') }}"></div>
                        <div class="field"><label>Grado de InstrucciÃ³n</label><input type="text" name="grado_instruccion" class="input-field" value="{{ old('grado_instruccion', $detalle->grado_instruccion ?? '') }}"></div>
                        <div class="field"><label>Fecha de Entrevista</label><input type="text" name="fecha_entrevista" class="input-field" value="{{ old('fecha_entrevista', isset($detalle->fecha_entrevista) ? \Carbon\Carbon::parse($detalle->fecha_entrevista)->format('d/m/Y') : '') }}" placeholder="DD/MM/AAAA" oninput="formatearFecha(this)" maxlength="10" autocomplete="off"></div>
                    </div>
                    ...
                    <div class="grid-form">
                        ...
                        <div class="field"><label>CÃ©dula</label><input type="text" name="cedula_representante" class="input-field" value="{{ old('cedula_representante', $detalle->cedula_representante ?? '') }}"></div>
                        <div class="field"><label>TelÃ©fono</label><input type="text" name="telefono_representante" class="input-field" value="{{ old('telefono_representante', $detalle->telefono_representante ?? '') }}"></div>
                        <div class="field"><label>Correo electrÃ³nico</label><input type="email" name="email_representante" class="input-field" value="{{ old('email_representante', $detalle->email_representante ?? '') }}"></div>
                        <div class="field"><label>Parentesco</label><select name="parentesco" class="input-field"><option value="">Seleccione...</option>@foreach(['Padre', 'Madre', 'Abuelo/a', 'TÃ­o/a', 'Hermano/a', 'Otro'] as $p)<option value="{{ $p }}" {{ old('parentesco', $detalle->parentesco ?? '') == $p ? 'selected' : '' }}>{{ $p }}</option>@endforeach</select></div>
                    </div>
                    ...
                    <div class="grid-form">
                        ...
                        <div class="field"><label>OcupaciÃ³n del Padre</label><input type="text" name="padre_ocupacion" class="input-field" value="{{ old('padre_ocupacion', $detalle->padre_ocupacion ?? '') }}"></div>
                        ...
                        <div class="field"><label>OcupaciÃ³n de la Madre</label><input type="text" name="madre_ocupacion" class="input-field" value="{{ old('madre_ocupacion', $detalle->madre_ocupacion ?? '') }}"></div>
                    ...
                    <div class="grid-form">
                        <div class="field full-width"><label>Motivo de consulta</label><textarea name="motivo_consulta" class="input-field" rows="4" placeholder="Â¿QuÃ© te trajo a consulta?">{{ old('motivo_consulta', $detalle->motivo_consulta ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Observaciones de los padres</label><textarea name="pads_observaciones" class="input-field" rows="4" placeholder="Â¿QuÃ© observan en casa?">{{ old('pads_observaciones', $detalle->pads_observaciones ?? '') }}</textarea></div>
                    ...
                    <div class="grid-form">
                        <div class="field full-width"><label>Â¿CuÃ¡ndo empezÃ³? Â¿Fue gradual o tras evento?</label><textarea name="cuando_empezo" class="input-field" rows="3">{{ old('cuando_empezo', $detalle->cuando_empezo ?? '') }}</textarea></div>
                    ...
                    <h4 style="color: #2d3748; margin: 25px 0 15px; font-size: 16px;">
                        <i class="fas fa-users" style="color: #f59e0b;"></i> Relaciones, Vida Social y AcadÃ©mica
                    </h4>
                    <div class="grid-form">
                        ...
                        <div class="field"><label>Rendimiento AcadÃ©mico</label><input type="text" name="rendimiento_academico" class="input-field" value="{{ old('rendimiento_academico', $detalle->rendimiento_academico ?? '') }}"></div>
                        <div class="field"><label>Â¿Tiene amigos?</label><input type="text" name="tiene_amigos" class="input-field" value="{{ old('tiene_amigos', $detalle->tiene_amigos ?? '') }}"></div>
                        ...
                        <div class="field full-width"><label>Â¿Rechazo, bullying o ciberacoso?</label><textarea name="rechazo_bullying" class="input-field" rows="2">{{ old('rechazo_bullying', $detalle->rechazo_bullying ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Pareja</label><textarea name="pareja" class="input-field" rows="2">{{ old('pareja', $detalle->pareja ?? '') }}</textarea></div>
                        ...
                        <div class="field full-width"><label>Â¿Mensajes que te hayan hecho daÃ±o?</label><textarea name="mensajes_dano" class="input-field" rows="2">{{ old('mensajes_dano', $detalle->mensajes_dano ?? '') }}</textarea></div>
                    </div>

                    <h4 style="color: #2d3748; margin: 25px 0 15px; font-size: 16px;">
                        <i class="fas fa-heartbeat" style="color: #f59e0b;"></i> Salud, Consumo y EvaluaciÃ³n Emocional
                    </h4>
                    <div class="grid-form">
                        <div class="field full-width"><label>Salud fÃ­sica</label><textarea name="salud_fisica" class="input-field" rows="3">{{ old('salud_fisica', $detalle->salud_fisica ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿Terapia anterior o medicaciÃ³n?</label><textarea name="terapia_anterior" class="input-field" rows="2">{{ old('terapia_anterior', $detalle->terapia_anterior ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Antecedentes Familiares</label><textarea name="antecedentes_familiares" class="input-field" rows="2">{{ old('antecedentes_familiares', $detalle->antecedentes_familiares ?? '') }}</textarea></div>
                        ...
                        <div class="field full-width"><label>Â¿Reducir consumo? Â¿Molestia por crÃ­ticas?</label><textarea name="reducir_consumo" class="input-field" rows="2">{{ old('reducir_consumo', $detalle->reducir_consumo ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿Usar para olvidar? Â¿Olvidar lo que pasÃ³?</label><textarea name="usar_para_olvidar" class="input-field" rows="2">{{ old('usar_para_olvidar', $detalle->usar_para_olvidar ?? '') }}</textarea></div>
                        <div class="field full-width"><label>OrientaciÃ³n sexual / Identidad de gÃ©nero</label><textarea name="orientacion_sexual" class="input-field" rows="2">{{ old('orientacion_sexual', $detalle->orientacion_sexual ?? '') }}</textarea></div>
                        <div class="field full-width"><label>DepresiÃ³n</label><textarea name="depresion" class="input-field" rows="3">{{ old('depresion', $detalle->depresion ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Ansiedad / PÃ¡nico</label><textarea name="ansiedad_panico" class="input-field" rows="3">{{ old('ansiedad_panico', $detalle->ansiedad_panico ?? '') }}</textarea></div>
                        ...
                        <div class="field full-width"><label>RelaciÃ³n con los Padres</label><textarea name="relacion_padres" class="input-field" rows="2">{{ old('relacion_padres', $detalle->relacion_padres ?? '') }}</textarea></div>
                    ...
                    <div class="grid-form">
                        <div class="field full-width"><label>Â¿QuÃ© haces bien? Â¿QuÃ© te da orgullo?</label><textarea name="que_haces_bien" class="input-field" rows="2">{{ old('que_haces_bien', $detalle->que_haces_bien ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿QuÃ© te da orgullo?</label><textarea name="que_te_da_orgullo" class="input-field" rows="2">{{ old('que_te_da_orgullo', $detalle->que_te_da_orgullo ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Â¿QuiÃ©n te respalda?</label><textarea name="quien_te_respalda" class="input-field" rows="2">{{ old('quien_te_respalda', $detalle->quien_te_respalda ?? '') }}</textarea></div>
                        <div class="field"><label>Escala 0-10: Â¿CuÃ¡nto quieres que cambien las cosas?</label><input type="number" name="cuanto_quieres_cambio" class="input-field" min="0" max="10" value="{{ old('cuanto_quieres_cambio', $detalle->cuanto_quieres_cambio ?? '') }}"></div>
                        <div class="field full-width"><label>3 valores + 1 acciÃ³n pequeÃ±a</label><textarea name="mini_ejercicio" class="input-field" rows="3">{{ old('mini_ejercicio', $detalle->mini_ejercicio ?? '') }}</textarea></div>
                        <div class="field full-width"><label>Observaciones</label><textarea name="observaciones" class="input-field" rows="3">{{ old('observaciones', $detalle->observaciones ?? '') }}</textarea></div>
                    </div>
                </div>
                @endif
                ...
                {{-- NIÃ‘O --}}
                @if($paciente->tipo_paciente === 'niÃ±o')
                <div id="ninoFields">
                    <h4 style="color: #2d3748; margin: 25px 0 15px; font-size: 16px;">
                        <i class="fas fa-info-circle" style="color: #10b981;"></i> Datos BÃ¡sicos
                    </h4>
                    <div class="grid-form">
                        <div class="field">
                            <label>Nombre </label>
                    ...
                    <div class="grid-form">
                        ...
                        <div class="field full-width">
                            <label>OcupaciÃ³n del Padre</label>
                        ...
                        <div class="field full-width">
                            <label>OcupaciÃ³n de la Madre</label>
                    ...
                    <div class="grid-form">
                        <div class="field">
                            ...
                            <select name="tipo_parto" class="input-field">
                                <option value="">Seleccione...</option>
                                @foreach(['Natural', 'CesÃ¡rea', 'Inducido'] as $tp)
                        ...
                        <div class="field full-width">
                            <label>AceptaciÃ³n Maternidad</label>
                    ...
                    <div class="grid-form">
                        <div class="field full-width">
                            <label>Desarrollo en Diferentes Ãreas</label>
                        ...
                        <div class="field full-width">
                            <label>AlimentaciÃ³n del BebÃ©</label>
                        ...
                        <div class="field full-width">
                            <label>Trastornos AlimentaciÃ³n</label>
                    ...
                    <h4 style="color: #2d3748; margin: 25px 0 15px; font-size: 16px;">
                        <i class="fas fa-school" style="color: #10b981;"></i> Ãrea Escolar y Conductual
                    </h4>
                    <div class="grid-form">
                        <div class="field full-width">
                        ...
                        <div class="field full-width">
                            <label>Responsabilidades DomÃ©sticas</label>
                        ...
                        <div class="field">
                            ...
                            <select name="finalizacion_tareas" class="input-field">
                                <option value="">Seleccione...</option>
                                @foreach(['SÃ­, siempre', 'A veces', 'No'] as $ft)
                    ...
                    <div class="grid-form">
                        ...
                        <div class="field full-width">
                            <label>Tipo de NiÃ±os que le Atraen</label>
                        ...
                        <div class="field full-width">
                            <label>RelaciÃ³n con Adultos</label>
...
function validarFormEdit() {
    ...
    if (!tipoAtencion || !tipoAtencion.value) {
        SIPCE_ALERT.warning('Seleccione el tipo de atenciÃ³n.');
    ...
    if (fechaInput && fechaInput.value.trim()) {
        if (!/^\d{2}\/\d{2}\/\d{4}$/.test(fechaInput.value)) {
            SIPCE_ALERT.warning('Ingrese una fecha vÃ¡lida en formato DD/MM/AAAA.');
            fechaInput.focus();
            return false;
        }
        ...
        if (dia < 1 || dia > diasPorMes[mes - 1]) {
            SIPCE_ALERT.warning('El dÃ­a ingresado no es vÃ¡lido.');
            fechaInput.focus();
            return false;
        }
        ...
        if (anio < 1900 || anio > anioActual) {
            SIPCE_ALERT.warning('El aÃ±o debe estar entre 1900 y ' + anioActual + '.');
            fechaInput.focus();
            return false;
        }
        ...
        if (tipoPaciente === 'niÃ±o' && edad > 11) {
            SIPCE_ALERT.warning('El paciente niÃ±o debe tener mÃ¡ximo 11 aÃ±os.');
            fechaInput.focus();
            return false;
        }

        if (tipoPaciente === 'adolescente' && (edad < 12 || edad > 17)) {
            SIPCE_ALERT.warning('El paciente adolescente debe tener entre 12 y 17 aÃ±os.');
            fechaInput.focus();
            return false;
        }
    }
